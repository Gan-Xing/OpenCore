...
| System 地区/IP                   | system        | `tool.area` / `integration.ip`                    | tool / integration     | none                             | partial  | P4         | yes               | yes                 | yes                | IP/location provider、GeoIP adapter、Area dataset version/query/import/Admin 已落地；完整行政区划运营平台后续准入 |
