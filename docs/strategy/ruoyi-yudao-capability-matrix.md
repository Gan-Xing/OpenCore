...
本矩阵基于 `/home/ubuntu/dev/ruoyi-vue-pro` 和 `/home/ubuntu/dev/yudao-ui-admin-vue3` 的模块地图，并结合 OpenCore 当前 BE20、Admin Pro V6、Cycle-021 closure、NestWeb、Antdpro6 旧项目经验做转译。矩阵用于说明 OpenCore 对若依/芋道能力的归属、优先级和当前完成状态；逐项验收证据以 Cycle-021 acceptance/waterline 文档为准，行业深水区仍必须单独准入。
