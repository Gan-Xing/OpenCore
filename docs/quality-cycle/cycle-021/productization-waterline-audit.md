...
| 5/17/18 | `core.role`           | First loop, enhance     | Role CRUD, permission-code assignment, data scope, role menu-tree assignment and role-user assignment are live. Round 17 revokes active sessions for role menu-permission mutation, and Round 18 revokes changed user-role sessions. Role status and broader user mutation flows remain. |
...
1. `core.role` plus `core.user`: role status, user status/reset-password flows
   and token/session refresh semantics after user-role/user mutation. Round 17
   completed role menu-tree assignment and role-permission session revocation.
   Round 18 completed role-user assignment and user-role session revocation.
