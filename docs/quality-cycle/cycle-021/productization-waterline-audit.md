...
2. `core.login-log`: Round 26 completed browser/OS parsing and server-side
   ...
   proving lockout can be driven from `auth.login.maxFailedAttempts`.
   Remaining login-log work is IP/location enrichment where feasible and
   broader logout/mobile/social logging stages.
