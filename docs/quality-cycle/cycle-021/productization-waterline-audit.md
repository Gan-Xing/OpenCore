...
| System Area (`system.area`)            | Meets            | Area tree, child/level query, detail, formatter, IP lookup, versioned dataset governance, import dry-run, activation, reusable AreaCascader, SDK, OpenAPI, local smoke, public API smoke, public Admin smoke and deploy guard are present.                |
