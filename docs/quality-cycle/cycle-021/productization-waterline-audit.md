...
| 11/26/45                                 | `core.login-log`      | First loop, enhance     | Immutable list/detail/export and failed-login smoke are live. Round 26 adds browser/OS device fields plus server-side IP and time-window filters. Round 45 adds persisted `logType/result` schema, username-login success/bad-credentials/disabled-user result mapping, API/SDK/Admin fields and server filters, export columns and smoke guards for invalid enum values. IP location enrichment and cleanup/unlock policy integration remain below reference depth.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
1. `core.config`: Round 24 completed public get-value-by-key plus cache
   ...
   guardrails. Remaining config work is any admitted secret vault/KMS
   integration, broader feature-flag propagation and real login lockout/unlock
   enforcement if that becomes the next security policy stage.
2. `core.login-log`: Round 26 completed browser/OS parsing and server-side
   ...
   Admin display and `logType/result` filters. Remaining login-log work is
   IP/location enrichment where feasible plus cleanup/unlock policy integration.
