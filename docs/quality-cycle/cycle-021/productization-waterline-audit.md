...
1. `core.user`: Round 19 completed user status/reset-password flows and direct
   ...
   Round 23 completed department side-tree filtering; Round 28 completed
   authenticated self-profile basic display-name read/update; Round 29
   completed authenticated self-password change with old-password verification
   and session revocation; Round 30 completed the authenticated user
   simple-list option source; Round 31 completed profile avatar upload,
   public preview, replacement and deletion backed by file storage; Round 32
   completed batch enable/disable and batch delete with session revocation;
   Round 33 completed CSV-compatible import template/import results with
   update-existing session revocation; Round 34 completed the dedicated
   `core:user:import` permission across registry/API/Admin/smoke; Round 35
   completed native XLSX export payload plus Admin download and smoke guards;
   Round 36 completed native XLSX import template/parsing while keeping CSV
   backwards compatibility. Remaining user work is any dedicated User-page
...
2. `core.config`: Round 24 completed public get-value-by-key plus cache
   ...
   invalidation. Remaining config work is broader runtime propagation
   boundaries and any admitted built-in-config deletion policy.
