...
1. `core.config`: Round 24 completed public get-value-by-key plus cache
   ...
   and smoke/deploy stale-bundle guards. Round 65 completed feature-flag
   audience targeting with public json `feature.*.audienceRules`, runtime
   ...
   External KMS provider binding, key rotation and secret version history
   remain later hardening stages.
...
4. `core.notice`: Round 55 completed persisted per-user read/unread state,
   ...
   fixed/deploy/public smoke. Round 66 completed the mail/SMS sandbox
   integration outbox bridge with channel-specific dispatch/execute bodies,
   provider readiness guards, `IntegrationOutbox` queue rows, delivery
   `recipient/providerMessageId`, Admin mail/SMS actions and smoke/deploy
   stale-bundle guards. Remaining notice work is real external SMTP/SMS
   provider execution/callbacks, WebSocket realtime push, multi-channel
   retry/failure queues and any admitted tenant/member/mobile channels before
   claiming full notice-product depth.
...
Round 65 added the config feature-audience guard: `core.config` smoke now
verifies seeded `feature.notice.inbox.audienceRules` as public json, runtime
`featureFlagRules.*.audienceRules`, attribute-aware evaluate results,
`audience-mismatch`, invalid audience create/update guards and matching
attributes for rollout checks. Admin static smoke and deploy guard reject stale
Config bundles missing `Audience Rules`, `Set audience` or `Feature audience`;
public verification proved the deployed Config chunk, OpenAPI markers and
public `config/feature-flags/evaluate` attributes path through the config
smoke.

Round 66 added the notice integration-outbox bridge guard: `core.notice` smoke
now dispatches mail and SMS delivery rows, proves a disabled integration
provider returns 400 before execution, enables the sandbox provider, executes
the provider bridge, verifies `queuedOutboxCount`, delivery
`recipient/providerMessageId` and matching `IntegrationOutbox` payload rows.
The round also added the missing integration runtime table migration because
`pnpm prisma:seed` exposed schema/migration drift. Admin static smoke and
deploy guard reject stale System Notices bundles missing `Dispatch mail
deliveries`, `Dispatch SMS deliveries`, `Execute mail outbox provider`,
`Execute SMS outbox provider` or `Provider Message`.
