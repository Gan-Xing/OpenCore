...
4. `core.notice`: Round 55 completed persisted per-user read/unread state,
   ...
   modal and read-status synchronization. Remaining notice work is real
   delivery adapter execution and any admitted WebSocket/mail/SMS fan-out
   before claiming full notice-product depth.
