...
3. `core.dept`: Round 27 completed the enabled-department simple-list option
   ...
   Remaining department work is data-scope workflow integration, plus batch
   department deletion or drag-sort UI only if separately admitted.
4. `core.notice`: read/unread state, inbox/header badge and delivery adapter
