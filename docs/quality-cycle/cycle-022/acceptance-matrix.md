...
| Database      | Tenant-owned repositories scoped by tenant                | Partial    | Role catalog is done in T3b. Dept/Post and broader System/core repositories remain T3/T4.                                            |
| Database      | Composite unique constraints for tenant-owned codes       | Partial    | Role `(tenantId, code)` is done in T3b. Dept/Post/Dict/Config uniqueness remains pending.                                            |
...
| Smoke         | Tenant foundation/auth/RBAC smokes exist                  | Done (T3b) | `pnpm smoke:core-tenancy-foundation`, `pnpm smoke:core-tenancy-auth`, `pnpm smoke:core-tenant-rbac`, and `pnpm smoke:core-tenant-role`. |
| Guard         | Tenant foundation/auth/RBAC guards exist                  | Done (T3b) | `pnpm guard:tenant-foundation`, `pnpm guard:tenant-auth`, `pnpm guard:tenant-rbac`, and `pnpm guard:tenant-role-scope`.              |
...
| Delivery      | lint/typecheck/test/smoke/deploy pass                     | Done (T3b) | Full lint/typecheck/test, deploy, local/public tenant foundation/auth/RBAC/role smokes, and public Admin checks passed.              |
