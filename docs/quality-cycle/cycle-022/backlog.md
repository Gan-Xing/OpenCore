...
| T3 organization and RBAC tenantization | In progress | tenant roles, departments, posts, membership role/post assignment, role permission clipping                                                 | T3a authenticates from membership role/post bridges and clips permissions by tenant plan modules. T3b makes Role tenant-owned; T3c makes Post tenant-owned. Tenant-owned Dept CRUD and non-root membership assignment remain. |
...
- T3c fixed Post catalog tenant scoping and post code uniqueness, but Department repositories are still globally keyed.
