...
- Completed sub-slices: T3a membership-scoped authz/plan clipping; T3b tenant-scoped Role catalog
...
| T3 organization and RBAC tenantization | In progress | tenant roles, departments, posts, membership role/post assignment, role permission clipping                                                 | T3a authenticates from membership role/post bridges and clips permissions by tenant plan modules. T3b makes Role tenant-owned with `(tenantId, code)` uniqueness. Tenant-owned Dept/Post CRUD and non-root membership assignment remain. |
...
- T3b fixed Role catalog tenant scoping and role code uniqueness, but Department/Post repositories are still globally keyed.
