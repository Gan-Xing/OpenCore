...
- `apps/*`：可运行应用。
- `packages/*`：共享库、契约、SDK、模块注册表和可复用 backend/runtime 包。
...
- `apps/admin`：Umi Max + Ant Design Pro V6 官方后台，已完成 S2-S8、Q001 和 Admin Pro V6 migration 主线。
...
当前主线仍只开发 api/admin；mobile、miniapp、desktop 保持占位，后续必须单独准入。

## packages

- `shared`：通用 validation result、runtime type guard、duplicate detection 等工具。
- `common`：后端通用常量、错误码、响应契约、分页/排序/filter helper。
- `contracts`：权限码、模块/menu/permission schema、OpenAPI snapshot、table export/query/upload/error/OpenForge contract。
- `sdk`：RBAC、系统管理、monitor/tool/collaboration/operations/integration typed clients 和 registry fixtures。
- `module-registry`：core、monitor、tool、collaboration、optional、integration 模块、权限、菜单、OpenAPI tag 和高风险业务泄漏检查。
...
- `monitor`：health、runtime diagnostics、status/version/queue/cache monitor。
...
- 已完成 S3-S9、runtime integration、OpenForge V1、Q001 和 Backend Self-Loop BE20；P4/P5 行业和高风险能力仍保留长期 backlog，不能绕过准入直接进入 core。
