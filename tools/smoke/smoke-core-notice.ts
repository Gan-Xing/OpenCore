...
const createdTemplateCodes: string[] = [];
...
async function main() {
  try {
    ...
    await request(`${apiPrefix}/core/notices/inbox/events`, {
      expected: [401],
    });
    ...
    createdNoticeIds.push(
      assertString(templateNotice.id, 'template notice id'),
    );
    assertEqual(
    ...
    const plainStartTlsCommandsBefore = countSmtpCommands(
      smtpServer,
      'STARTTLS',
    );
    await apiRequest(
    ...
    assertNumberAtLeast(
      smtpProcess.sentCount,
      1,
      'Mail SMTP process sent count',
    );
    ...
    assertEqual(
      cappedSmsSchedule.retriedCount,
      0,
      'SMS capped schedule retries',
    );
    assertEqual(
    ...
    assertEqual(
      smsHttpOutbox.status,
      'queued',
      'SMS HTTP outbox queued status',
    );
    const smsHttpProcess = await apiRequest(
    ...
    assertEqual(
      sentSmsHttpOutbox.status,
      'sent',
      'SMS HTTP outbox sent status',
    );
    ...
    const readRealtimeStream = await openSseStream(
      '/core/notices/inbox/events',
    );
...
async function startSmokeSmsHttpServer({ apiKey }: any) {
  const requests: any[] = [];
...
function readHttpRequestBody(request: http.IncomingMessage) {
...
async function startSmokeSmtpServer({ password, username }: any) {
